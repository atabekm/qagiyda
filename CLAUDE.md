...
**Search is local and dependency-free.** `search.json` is a Liquid template that splits each rule's rendered HTML on `</p>` and emits one record per paragraph (`u`rl, `p`aragraph index, `t`itle, `s`ection, `c`ontent) — currently 129 records, ~42 KB. `assets/js/search.js` fetches it once on first focus and does substring matching in the browser. Nothing is precomputed at deploy time beyond the normal `jekyll build`.

**Results deep-link to the matching paragraph**, which couples two files that must be changed together: `search.json` numbers each paragraph, and `_layouts/post.html` re-emits the rule body with a matching `<span class="p-anchor" id="pN">` before each block. Both must use the *identical* split rule — split on `</p>`, discard blocks whose stripped text is empty — or every anchor past the divergence points at the wrong paragraph. `post.html` appends an `<!--e-->` sentinel before splitting so the blocks can be rejoined correctly whether or not the body ends in `</p>`; without it the final `</p>` is either dropped or doubled. A block is not always one element (a `<ul>` contains no `</p>`, so it merges with the paragraph that follows it), which is why the arrival highlight in `search.js` walks siblings until the next `.p-anchor` rather than styling a single node.

To verify alignment after touching either file: build, then for each `_site/rules/*/index.html` split the `<article>` on the anchor spans and check each fragment's stripped text equals the `c` of the record with that `p`. All 129 currently match exactly.
